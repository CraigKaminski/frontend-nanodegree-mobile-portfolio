...
var resizePizzas = function(size) {
  ...
  function changePizzaSizes(size) {
    // Instead of repeating the same query each time through the loop, perform it once
    // and save the results
    var randomPizzaContainerElements = document.querySelectorAll(".randomPizzaContainer");
    // Since each pizza element is the same size, no need to determine the size difference
    // and new size for each one. Instead we do it once and use the results for the
    // rest of the pizza elements.
...
// Instead of getting the pizzasDiv element every time through the loop, get it only once.
...
document.addEventListener('DOMContentLoaded', function() {
  var cols = 8;
  var s = 256;
  // Instead of drawing 200 pizzas, only draw number of pizzas
  // required to fill the screen.
  var numPizzas = Math.ceil(screen.height / s) * cols;
